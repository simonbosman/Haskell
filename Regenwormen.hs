...
pakUitStapelsTegels sc sts = do let hts  = [head st | st <- sts]
                                let t    = filter (\(t,w) -> sc == t) hts
                                if t /= [] then
                                  do let sid = fromJust $ elemIndex (head t) hts
                                     let stn = stapelTegels [POP] (sts !! sid)
                                     let stsn = take sid sts ++ stn : drop (sid+1) sts
                                     (stsn,head t)
                                else
                                  (sts,(0,0))
...
speel' _ _ _ [] sts     = do let wt = [waardeStapelTegels st | st <- sts]
                             let hsc = maximum wt
                             let sp = (\(Just n)->n) (findIndex (hsc==) wt) + 1
                             putStrLn $ "speler: " ++ show sp ++ " heeft gewonnen, score: " ++ show hsc
                                   ...
                                   do let (stsn,tn) = pakUitStapelsTegels sc sts 
                                      if fst tn == 0 then
                                        ...
                                        do let st = stapelTegels [PUSH tn] (stsn !! sid)
                                           speel' sp (volgendeSpeler sid) p rtsn (stapelsTegels st stsn)
                                
                                 else 
                                   do let st = stapelTegels [PUSH t] (sts !! sid)
                                      speel' sp (volgendeSpeler sid) p rtsn (stapelsTegels st sts)
